...
> [`@sproutpad/conformance@0.2.3`](https://www.npmjs.com/package/@sproutpad/conformance)
...
>
> **Dialect:** `@0.2.3` grades the typed-actions agent envelope (spec formatVersion 2) and compares MCP descriptions to the transport binding published by the catalog. `@0.1.1` expects the retired prose `nextActions` / agent `resolution` dialect and will fail a v2 governor.
>
> **`@0.1.0` was unpublished.** That release had a broken CLI entrypoint
> (silent exit under `npx`). Use `@0.2.3` only.
...
```bash
npx @sproutpad/conformance@0.2.3 --base-url https://api.sproutpad.ai
...
```bash
npx @sproutpad/conformance@0.2.3 \
  --profile wire \
...
npx @sproutpad/conformance@0.2.3 verify bundle.json
...
npx @sproutpad/conformance@0.2.3 verify governed-bundle.json
```
...
npx @sproutpad/conformance@0.2.3 verify bundle.json --jwks jwks.json
```
...
} from "@sproutpad/conformance/verify";
...
The governed loop stages one tiny static HTML bundle and deploys it through
the target's Cloudflare edge primitive. It does not use a service-starter
catalog or an unattended container-image approval. The target must therefore
have its edge deploy capability enabled; an unavailable edge target fails the
governed result rather than falling back to a different deploy path.
