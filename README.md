...
> [`@sproutpad/conformance@0.2.2`](https://www.npmjs.com/package/@sproutpad/conformance)
...
>
> **Dialect:** `@0.2.2` grades the typed-actions agent envelope (spec formatVersion 2) and compares MCP descriptions to the transport binding published by the catalog. `@0.1.1` expects the retired prose `nextActions` / agent `resolution` dialect and will fail a v2 governor.
>
> **`@0.1.0` was unpublished.** That release had a broken CLI entrypoint
> (silent exit under `npx`). Use `@0.2.2` only.
...
```bash
npx @sproutpad/conformance@0.2.2 --base-url https://api.sproutpad.ai
...
```bash
npx @sproutpad/conformance@0.2.2 \
  --profile wire \
...
npx @sproutpad/conformance@0.2.2 verify bundle.json
...
npx @sproutpad/conformance@0.2.2 verify governed-bundle.json
```
...
npx @sproutpad/conformance@0.2.2 verify bundle.json --jwks jwks.json
```
