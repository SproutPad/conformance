...
// This matches the protected public-conformance job's hard timeout and the
// API ingestion bound, so offline verification accepts all retained evidence
// that the API can ingest.
export const MAX_CONFORMANCE_REPORT_DURATION_MS = 100 * 60_000;
...
export function parseReport(value) {
  ...
  if (
    ...
    completed < started ||
    completed - started > MAX_CONFORMANCE_REPORT_DURATION_MS
  ) {
    throw new ConformanceError(
      "conformance:time",
...
export async function verifyConformanceBundle(input, jwks, policy = {}) {
  ...
  const reportPassed = report?.summary.outcome === "pass";
  if (report && !reportPassed) {
    errors.push("conformance:outcome: Report outcome is not pass");
  }
  return {
    ok:
